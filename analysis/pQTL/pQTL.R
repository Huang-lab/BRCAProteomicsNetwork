##### pQTL.R #####
# Kuan-lin Huang
# find protein associated with mutations
# function called by other scripts

library(limma)

# function
diff_exp = function(m, g1, g2, name="data", limma=F){
  ...
  stats = matrix(,nrow=nrow(x),ncol=8)
  row.names(stats)=row.names(x)
  colnames(stats) = c("name",paste(m.n,g1.n,"mean",sep="_"), paste(m.n,g2.n,"mean",sep="_"), "meanChange","t_test_Tstat", "t_test_p", "w_test_Wstat", "w_test_p")
  for (i in 1:nrow(x)){
    x1 = as.numeric(x[i,])
    y1 = as.numeric(y[i,])
    ...
    if (sum(!is.na(x1))<2 | sum(!is.na(y1))<2){
      stats[i,]=c(name, meanx,meany,change,rep("NA",4))} 
    else{
      # t-test
      t = try(t.test(x1,y1), silent=T)
      if (is(t,"try-error")){ stats[i,] = c(name,meanx,meany,change,rep("NA",4)); next}
      t.p = t$p.value
      t.tstat = t$statistic
      ...
      w.Wstat = w$statistic
      # return the results
      stats[i,] = c(name, meanx, meany, change, t.tstat, t.p, w.Wstat, w.p)
    }
  }
  ...
  #tn = paste("out",name,m.n,g1.n,"vs",g2.n,"diff_exp.txt", sep="_")
  tn = paste("out/",name,"_pQTL.txt", sep="")
...
plot_diff_exp_heatmap = function(fold_change, t_fdr, fdr_cutoff=0.05, name="data"){
  ...
  fn = paste("out", name, 'diff_exp.pdf',sep ="_")
...
plot_diff_exp_violin = function(mut_exp, gene,name="data"){ ### select only the key mutations and the differentially expressed genes
  ...
  fn = paste("out", name, gene, "mutational_impact_violin.pdf", sep="_")
